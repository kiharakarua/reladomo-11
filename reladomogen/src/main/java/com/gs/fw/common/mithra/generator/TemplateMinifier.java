...
public class TemplateMinifier {
  ...
  private void writeMulti(StringBuilder builder, WriteMethod method, int multiCallCount,
      int multiCountStart) {
    if (multiCallCount > 1) {
      builder.append("out.writeMany").append(multiCallCount).append('(')
          .append(method.bufferedWrites.get(multiCountStart).buffer);
  ...
  private void writeBuffered(ArrayList<BufferedWrite> lineBuffer,
      ArrayList<WriteMethod> methodBuffer, StringBuilder builder) {
  ...
  private String cleanWrite(String argument, StringBuilder tempArg,
      ArrayList<BufferedWrite> lineBuffer) {
  ...
  private String replaceServletStuff(String line) {
    ...
    line = line.replace("implements org.apache.jasper.runtime.JspSourceDependent",
        "implements MithraTemplate");
